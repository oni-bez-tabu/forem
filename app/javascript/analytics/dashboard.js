...
function writeCards(data, timeRangeLabel, totals) {
  ...
  readerCard.innerHTML = readerCardHTML(readers, avgReadTime, `Czytelników ${timeRangeLabel}`);
  reactionCard.innerHTML = reactionCardHTML(reactions, uniqueReactors, `Reakcji ${timeRangeLabel}`);
  commentCard.innerHTML = cardHTML(comments, `Komentarzy ${timeRangeLabel}`);
  bookmarkCard.innerHTML = cardHTML(bookmarks, `Zakładek ${timeRangeLabel}`);
  if (followersCard) {
    const engagementEl = followersCard.querySelector('.follower-engagement');
    followersCard.innerHTML = cardHTML(sumAnalytics(data, 'follows'), `Obserwujących ${timeRangeLabel}`);
...
function drawCharts(data, timeRangeLabel) {
  ...
  drawChart({
    ...
    series: [
      { name: 'Wszystkie', data: reactionsExclBookmarks },
      { name: 'Polubienia', data: likes },
      { name: 'Unicorn', data: unicorns },
      { name: 'Exploding Heads', data: explodingHeads },
      { name: 'Raised Hands', data: raisedHands },
      { name: 'Ogień', data: fires },
      { name: 'Zakładki', data: readingList },
    ],
  });
  ...
  drawChart({
    ...
    isInfinity,
    colors: ['#4bc0c0'],
    series: [{ name: 'Komentarze', data: comments }],
  });
  ...
  drawChart({
    ...
    strokeDashArray: [0, 4],
    series: [
      { name: 'Czytelnicy', data: readers },
      { name: 'Śr. czas czytania (s)', data: avgReadTime },
    ],
    yaxis: [
      {
        min: 0,
        title: { text: 'Czytelnicy', style: { color: '#9d39e9', fontSize: '12px' } },
        labels: { formatter: (val) => Math.round(val) },
      },
      {
        opposite: true,
        min: 0,
        title: { text: 'Śr. czas czytania (s)', style: { color: '#10b981', fontSize: '12px' } },
...
function renderReferrers(data) {
  ...
  if (emptyDomainReferrer) {
    tableBody.push(`
      <tr>
        <td class="align-left">Wszystkie inne zewnętrzne strony</td>
